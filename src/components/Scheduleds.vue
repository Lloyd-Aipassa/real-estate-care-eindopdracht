...
export default {
  ...
  methods: {
    getData() {
      this.loading = true
      fetch('https://first-server-production.up.railway.app/scheduled?_sort=Datum&order=desc')
        .then((res) => res.json()
    ...
    deleteRapport(id) {
      ...
      axios.delete('https://first-server-production.up.railway.app/scheduled' + id)
